...
def test_performance_testing_constants():
    ...
    try:
        from streaming_analytics.performance_testing import (
            QUANTILES_COUNT, P95_QUANTILE_INDEX, P99_PERCENTILE
        )
        
        # Verify the constants have expected values
        assert QUANTILES_COUNT == 20, f"Expected QUANTILES_COUNT=20, got {QUANTILES_COUNT}"
        assert P95_QUANTILE_INDEX == 18, f"Expected P95_QUANTILE_INDEX=18, got {P95_QUANTILE_INDEX}"
        assert P99_PERCENTILE == 0.99, f"Expected P99_PERCENTILE=0.99, got {P99_PERCENTILE}"
...
def test_encryption_key_validation():
    ...
    try:
        ...
        try:
            manager = KafkaSecurityManager(config)
...
def test_proactive_cleanup_logic():
    ...
    try:
        ...
        with open('libs/streaming_analytics/processor.py', 'r') as f:
            content = f.read()
...
def test_exception_handling_improvements():
    ...
    try:
        ...
        with open('libs/streaming_analytics/kafka_manager.py', 'r') as f:
            content = f.read()
        
        # Check for specific exception types
        if ('except (ValueError, UnicodeDecodeError)' in content and 
            'except json.JSONDecodeError' in content):
...
def test_realtime_ml_refactoring():
    ...
    try:
        ...
        with open('libs/streaming_analytics/realtime_ml.py', 'r') as f:
            content = f.read()
        ...
        found_methods = sum(1 for method in helper_methods if f'def {method}(' in content)
        
        if found_methods >= 4:  # Should have most of the helper methods
            print(f"✓ Realtime ML refactoring implemented ({found_methods}/{len(helper_methods)} methods found)")
            return True
        else:
            print(f"✗ Realtime ML refactoring incomplete ({found_methods}/{len(helper_methods)} methods found)")
            return False
