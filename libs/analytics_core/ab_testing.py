...
class ABTestingEngine:
    ...
    def create_experiment(
        self, experiment: ABTestExperiment, auto_create_feature_flag: bool = True
    ) -> ABTestExperiment:
        ...
        try:
            ...
            if flag:
                self.feature_flag_manager.update_flag(
                    experiment.feature_flag_key, {"status": FeatureFlagStatus.ACTIVE}
                )
            ...
            if experiment.mlflow_experiment_id:
                ...
                self.experiment_tracker.start_run(
                    experiment_id=experiment.mlflow_experiment_id, metadata=run_metadata
                )
    ...
    def get_feature_value(
        ...
        try:
            ...
            variant_name = self.assign_user_to_variant(
                experiment_id, user_id, user_attributes
            )
            if not variant_name:
                return None
    ...
    def analyze_experiment(
        self, experiment_id: str, test_type: TestType = TestType.TWO_SAMPLE_TTEST
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if len(variant_names) > 2:
                p_values = [
                    test["p_value"] for test in results["statistical_tests"].values()
                ]
                if p_values:
                    correction = (
                        self.statistical_analyzer.multiple_comparison_correction(
                            p_values,
    ...
    def stop_experiment(self, experiment_id: str, reason: str | None = None) -> bool:
        ...
        try:
            ...
            self.feature_flag_manager.update_flag(
                experiment.feature_flag_key, {"status": FeatureFlagStatus.INACTIVE}
            )
                ...
                self.experiment_tracker.log_metrics(
                    {
                        ...
                        "total_assignments": len(
                            self.assignments.get(experiment_id, [])
                        ),
    ...
    def list_experiments(
        self, status: ExperimentStatus | None = None, created_by: str | None = None
