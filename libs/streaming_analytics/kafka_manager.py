        ...
        try:
            ...
            for topic, f in fs.items():
                try:
                    ...
                    self.logger.error("Failed to delete topic", topic=topic, error=str(e))
                    return False
...
class KafkaProducerManager:
    ...
    async def send_event(self,
                        ...
                        headers: dict[str, bytes] | None = None) -> bool:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            'sent_count': self._sent_count,
            'error_count': self._error_count,
            'success_rate': self._sent_count / max(self._sent_count + self._error_count, 1)
        }
...
class KafkaConsumerManager:
    ...
    def add_handler(self, event_type: str, handler: Callable[[EventSchema], None]) -> None:
        """Add an event handler for specific event type."""
        if event_type not in self._handlers:
            self._handlers[event_type] = []
        self._handlers[event_type].append(handler)

        self.logger.info("Event handler added",
                        event_type=event_type,
                        handler_count=len(self._handlers[event_type]))

    def remove_handler(self, event_type: str, handler: Callable[[EventSchema], None]) -> None:
        ...
        if event_type in self._handlers:
            try:
                ...
                self.logger.warning("Handler not found for removal", event_type=event_type)
    ...
    async def _process_event(self, event: EventSchema, headers: dict[str, bytes]) -> None:
        ...
        if not handlers:
            self.logger.debug("No handlers registered for event type", event_type=event_type)
            return
        ...
        if tasks:
            try:
                ...
                self.logger.debug("Event handlers completed",
                                event_type=event_type,
...
class KafkaManager:
    ...
    async def get_consumer(self, group_id: str, topics: list[str]) -> KafkaConsumerManager:
        ...
        if consumer_key not in self.consumers:
            ...
            self.logger.info("Created new Kafka consumer",
                           group_id=group_id,
                           topics=topics)
